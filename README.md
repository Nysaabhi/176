...
processFlightQuery(query) {
  ...
  // Improved flight pattern to capture more variations
  const flightPattern = /(?:book|search|find)?\s?(?:a )?flight\s?(?:from )?([\w\s]+)?\s?(?:to )?([\w\s]+)?\s?(?:on )?(\d{1,2}(?:th|st|nd|rd)?\s?\w+)?\s?(?:and return on )?(\d{1,2}(?:th|st|nd|rd)?\s?\w+)?/i;
  const match = normalizedQuery.match(flightPattern);

  if (match) {
    const fromCity = match[1] ? match[1].trim().toLowerCase() : null;
    const toCity = match[2] ? match[2].trim().toLowerCase() : null;
    ...
    // Determine trip type based on whether a return date is provided
    const tripType = returnDate ? 'round-trip' : 'one-way';

    // Pass city names (not airport codes) to createFlightSearchResponse
    return this.createFlightSearchResponse(fromCity, toCity, departureDate, returnDate, classType, travellers, tripType);
  }

  // If the query is just "search flight" or similar, return the flight search interface
...
parseDate(dateString) {
  ...
  // Check if the dateString is a relative date
  if (relativeDates.hasOwnProperty(dateString)) {
    const today = new Date();
    today.setDate(today.getDate() + relativeDates[dateString]);
    return today.toISOString().split('T')[0];
  }

  // Handle dates like "21st March", "March 21st", or "21/03/2023"
  const dateParts = dateString.split(/\s+/);
  let day, month, year;

  if (/\d+/.test(dateParts[0])) {
    day = dateParts[0].replace(/\D/g, '');
    month = months[dateParts[1]?.toLowerCase()] || '01';
  } else if (/\d+/.test(dateParts[1])) {
    day = dateParts[1].replace(/\D/g, '');
    month = months[dateParts[0]?.toLowerCase()] || '01';
...
createFlightSearchResponse(fromCity = null, toCity = null, departureDate = null, returnDate = null, classType = 'e', travellers = 1, tripType = 'one-way') {
  ...
  const generateCityOptions = (selectedCity) => {
    return Object.keys(airportData)
      .map(city => `<option value="${airportData[city]}" ${selectedCity === city ? 'selected' : ''}>${city.charAt(0).toUpperCase() + city.slice(1)}</option>`)
      .join('');
  };
  ...
  const flightSearchHtml = `
    <div class="flight-search-response">
      ...
      <form id="flightSearchForm">
        ...
        <div class="form-group" id="returnDateGroup" style="${isRoundTrip ? '' : 'display: none;'}">
        ...
        <div class="form-group">
          ...
          <input type="number" id="travellers" name="travellers" value="${travellers}" min="1" required>
        </div>
  ...
  setTimeout(() => {
    document.getElementById('flightSearchForm').addEventListener('submit', (e) => {
      ...
      const url = this.generateFlightSearchUrl(from, to, departureDate, returnDate, classType, travellers, tripType);
...
generateFlightSearchUrl(from, to, departureDate, returnDate, classType, travellers, tripType) {
